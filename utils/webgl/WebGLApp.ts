...
class DefaultSizeProvider {
  get width() {
    return (window ?? null)?.innerWidth;
  }

  get height() {
    return (window ?? null)?.innerHeight;
  }

  get pixelRatio() {
    return Math.min((window ?? null)?.devicePixelRatio, 2);
  }
}
...
export default class WebGLApp {
  ...
  constructor(options: WebGLAppOptions = {}) {
    ...
    (window ?? null)?.addEventListener("resize", this._onResize);
  ...
  dispose() {
    ...
    (window ?? null)?.removeEventListener("resize", this._onResize);
